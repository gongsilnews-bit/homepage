...
export default function PropertyCard01({ property }: PropertyCardProps) {
  ...
  return (
    <a href={`/theme01/${property.id}`} className="block group">
      {/* Image Slider */}
      <div className="relative aspect-[4/3] rounded-lg overflow-hidden bg-gray-200">
        ...
        <div className="absolute top-3 right-3 flex gap-1.5">
          ...
          {showNewBadge && (
            <span className="bg-gold text-white text-[11px] font-bold px-2.5 py-1 rounded">
              NEW
            </span>
          )}
        </div>
        ...
        <button
          onClick={toggleLike}
          className="absolute bottom-3 right-3 text-white hover:text-gold transition-colors"
          aria-label="찜하기"
        >
          <FiHeart
            size={22}
            className={liked ? 'fill-gold text-gold' : ''}
          />
        </button>
            ...
            {property.images.map((_, idx) => (
              <div
                ...
                className={`w-1.5 h-1.5 rounded-full transition-all ${
                  idx === currentImage ? 'bg-gold w-3' : 'bg-white/60'
                }`}
              />
            ))}
          </div>
      ...
      <div className="mt-3 px-0.5">
        <div className="flex items-start gap-2 mb-1">
          ...
          <div className="flex items-center gap-1">
            <FaMapMarkerAlt className="text-gold shrink-0" size={12} />
